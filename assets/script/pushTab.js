export function pushPageTable(arrData, containerElDom, page) {
  ...
  arrData.map((element, index) => {
    if ((index >= firstIndexElement) & (index < lastIndexElement)) {
      let stateStatus =
        element.status === "Active" ? "is-active" : "is-inactive";
      const fragmentTbodyTr = `<tr class="main-costumers-tbody-tr">
			 ...
			 <td class="main-costumers-td"> 
					 <button class="main-costumers-btn ${stateStatus}" > ${element.status}</button>
			 </td> 
		 </tr>`;
